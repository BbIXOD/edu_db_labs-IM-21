...
export const test = async (request: FastifyRequest, reply: FastifyReply): Promise<string> => {
  const [_result] = await app.mysql.query('SELECT * FROM category')
  return 'ok'
}

export const getCategory = async (_request: FastifyRequest, reply: FastifyReply): Promise<RowDataPacket[]> => {
  const result = await app.mysql.query('SELECT * FROM category')

  return result[0] as RowDataPacket[]
}

export const getCategoryByID = async (request: FastifyRequest, reply: FastifyReply): Promise<RowDataPacket> => {
  const id = (request.params as { id: string }).id
  const dbReply = await app.mysql.query('SELECT * FROM category WHERE id = ?', [id])

  return dbReply[0] as RowDataPacket
}

export const addNewCategory = async (request: FastifyRequest, reply: FastifyReply): Promise<string> => {
...
export const extendCategory = async (request: FastifyRequest, reply: FastifyReply): Promise<string> => {
  ...
  if (result[0].length === 0) {
    return await reply.code(404).send('Category not found')
  }
...
export const deleteCategory = async (request: FastifyRequest, reply: FastifyReply): Promise<string> => {
  const id = (request.params as { id: string }).id

  const result = await app.mysql
    .execute('DELETE FROM category WHERE id = ?', [id]) as ResultSetHeader[]

  if (result[0].affectedRows === 0) return await reply.code(404).send('Category not found')
  return 'Category deleted successfully'
}

export const modifyCategory = async (request: FastifyRequest, reply: FastifyReply): Promise<string> => {
  const id = (request.params as { id: string }).id
